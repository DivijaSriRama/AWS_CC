...
def defaultLogin(conn):
    ...
    try:
        ...
        with loginPlaceholder.form("login"):
            ...
            if login_button:
                user_data = runQuery(
                    f"SELECT * FROM users_details WHERE u_name = '{username}' AND pwd = '{password}'", conn)
    ...
    try:
        ...
        with registerColInFileUpload:
            ...
            if navBarRegisterInFileUpload:
                ...
                st.experimental_rerun()
        st.title("Login Page")
        fetchUserDetailsQuery = f"SELECT f_name, l_name, mail_id FROM login_users WHERE u_name = '{st.session_state.username}'"
